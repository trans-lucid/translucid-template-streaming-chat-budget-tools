...
if ! grep -Eq "streaming_not_implemented|budget_exceeded_without_guard|tool_cache_miss" <<<"$MAIN_OUTPUT"; then
...
npm ci
EVAL_TARGET="$PWD/solution" npx vitest run --config vitest.config.ts tests/public/public.test.ts solution/tests/*.test.ts evaluator/tests_hidden/*.test.ts
